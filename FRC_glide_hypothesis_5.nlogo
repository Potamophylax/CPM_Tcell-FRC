globals [ EJ EJ0 Bound Bound_0 Ebound Ebound0 Surf Surf_0 Esurf Esurf0 Xc Yc Xc0 Yc0 DH PROB ENERGY
  delta_r
  alpha
  THETA        ; Угол направления силы
  THETA0       ; Угол направления силы с предыдущего шага
  THETA00      ; Средний угол направления клетки с предыдущих тау шагов Монте-Карло
  Conc         ; Концентрация хемокина в таргентном сайте
...
[ NR
  ...
  chem         ; Концентрация хемокина для движения по градинту
  TRACE
]

to setup
  clear-all
  ask patches [ set pcolor blue set TRACE 0]
  setup-rails
  setup-grad
  ask patch -48 -6 [ ask patches in-radius 10 [ set pcolor red ] ]
  ...
  set EJ0 ENERGY
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ; Задаём первые значения, чтобы сразу список направлений
  ; был для усреднения в угол для направения силы
  set alpha_list [0]
  repeat tau * N_iter [ set alpha_list lput THETA0 alpha_list ]
...
to setup-rails
  ...
  repeat 23
    [
      ask patches with [pycor = -0.25 * abs (pxcor) + h ] [set pcolor grey]
      ask patches with [pycor = -0.25 * abs (pxcor + 1) + h ] [set pcolor grey]
      ask patches with [pycor = -0.25 * abs (pxcor + 2) + h ] [set pcolor grey]
      ask patches with [pycor = -0.25 * abs (pxcor + 3) + h ] [set pcolor grey]
      set h (h + 1)
    ]
  connect-patches
end
...
to go
  ...
  repeat N_iter
  [
    let target one-of patches with [pcolor = grey] with [count neighbors with [pcolor = red] > Cc]
    ask target [set pcolor red]

    measure-energy
    set EJ ENERGY

    set Conc [chem] of target
    ask target [ ask one-of neighbors with [pcolor = red] [ set Conc0 chem] ]
    ...
    mark-centre
    set delta_r sqrt( (Xc - Xc0) ^ 2 + (Yc - Yc0) ^ 2 )
    set alpha 90 - atan (Yc - Yc0) (Xc - Xc0)

   set DH LAMBDA_PER * (Ebound - Ebound0) +
          LAMBDA_AR * (Esurf - Esurf0) +
          (EJ - EJ0) +
          LAMBDA_Pr * delta_r * cos (alpha - ( - THETA + 270)) -
          LAMBDA_GRAD * (Conc - Conc0)
    ...
    [
      ifelse PROB > random-float 1
        [ set pcolor red
          set alpha_list lput alpha alpha_list
        ]
        [ set pcolor grey ]
    ]
...
;======================================================================================
    ...
    let target2 one-of patches with [pcolor = red] with [count neighbors with [pcolor = grey] > Cc]
    ask target2 [set pcolor grey]

    measure-energy
    set EJ ENERGY

    set Conc [chem] of target2
    ask target2 [ ask one-of neighbors with [pcolor = grey] [ set Conc0 chem] ]
...
to-report perim [col1]
  ask patches with [pcolor = col1] [set NR count neighbors4 with [pcolor != red]]
...
to calculate-angle
  let NOISE random-normal 0 1
  let flip_change random tau

  set theta_list ifelse-value ( length alpha_list > tau * N_iter)
                              [sublist alpha_list (length alpha_list - tau * N_iter) (length alpha_list) ]
                              [alpha_list]
  set THETA00 mean theta_list
  ;; Stochastic equation solution
  set THETA ifelse-value
  (flip_change = 0)
  [phi * THETA00 + delta_theta * NOISE]
  [THETA00]
end
...
to measure-energy
  ...
  [
    set energy_i ;( count neighbors4 with [pcolor = grey ] ) * Jcf +
                 ( count neighbors4 with [pcolor = blue ] ) * Jcm
  ]
...
to show-trajectory
  ...
  [
    ...
    ask patches with [TRACE = 1] [set pcolor yellow]
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
