globals [ EJ EJ0 Bound Bound_0 Ebound Ebound0 Surf Surf_0 Esurf Esurf0 Xc Yc Xc0 Yc0 Edir Edir0 DH PROB ENERGY kdir GN MOVE_index Erail0 Erail Conc Conc0 Echem Echem0]
...
to setup-rails3
  ask patches with [pycor = -1 * (pxcor) - 30 ] [set pcolor grey]
  ;ask patches with [pycor = -0.25 * abs (pxcor + 1) - 10 ] [set pcolor grey]
  ;ask patches with [pycor = -0.25 * abs (pxcor + 2) - 10 ] [set pcolor grey]
  ;ask patches with [pycor = -0.25 * abs (pxcor + 3) - 10 ] [set pcolor grey]
  connect-patches
...
;======================================================================================
  ...
  ask target2 [ ask one-of neighbors with [pcolor = blue] [ set Conc0 chem] ]          ;; Проанализировать, можно ли получить изначальную концентрацию лучше?
