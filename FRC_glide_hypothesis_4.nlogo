...
to setup-chemotaxis
  ask patches with [pcolor = blue] ;with [count neighbors with [pcolor = grey] = 0]
    [
      let shadow patches with [pcolor = grey] in-radius 10
      ifelse not any? shadow
        [ set num_i 0]
        [ set num_i mean [pnum] of shadow ]
    ]
end
...
;  ]
  ...
  ;=============
  ifelse GN > 5
  [ set MOVE_index mean [num_i] of patches with [pcolor = red]]
  [ set MOVE_index 0]
